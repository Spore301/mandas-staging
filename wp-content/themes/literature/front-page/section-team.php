<div class="front_page_section front_page_section_team<?php
	$literature_scheme = literature_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $literature_scheme ) && ! literature_is_inherit( $literature_scheme ) ) {
		echo ' scheme_' . esc_attr( $literature_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( literature_get_theme_option( 'front_page_team_paddings' ) );
	if ( literature_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$literature_css      = '';
		$literature_bg_image = literature_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$literature_anchor_icon = literature_get_theme_option( 'front_page_team_anchor_icon' );
	$literature_anchor_text = literature_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( literature_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$literature_css      = '';
			$literature_bg_mask  = literature_get_theme_option( 'front_page_team_bg_mask' );
			$literature_bg_color_type = literature_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $literature_bg_color_type ) {
				$literature_bg_color = literature_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$literature_caption = literature_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $literature_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $literature_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $literature_caption, 'literature_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$literature_description = literature_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $literature_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $literature_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $literature_description ), 'literature_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! literature_exists_trx_addons() ) {
						...
						literature_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
