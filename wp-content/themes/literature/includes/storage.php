...
if ( ! function_exists( 'literature_storage_get' ) ) {
	 ...
	 */
	function literature_storage_get( $var_name, $default = '' ) {
		global $LITERATURE_STORAGE;
		return isset( $LITERATURE_STORAGE[ $var_name ] ) ? $LITERATURE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'literature_storage_set' ) ) {
	/**
	 ...
	 */
	function literature_storage_set( $var_name, $value ) {
		global $LITERATURE_STORAGE;
		$LITERATURE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'literature_storage_empty' ) ) {
	/**
	 ...
	 */
	function literature_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $LITERATURE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $LITERATURE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $LITERATURE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'literature_storage_isset' ) ) {
	/**
	 ...
	 */
	function literature_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $LITERATURE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $LITERATURE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $LITERATURE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'literature_storage_unset' ) ) {
	/**
	 ...
	 */
	function literature_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $LITERATURE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $LITERATURE_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
				unset( $LITERATURE_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'literature_storage_inc' ) ) {
	/**
	 ...
	 */
	function literature_storage_inc( $var_name, $value = 1 ) {
		global $LITERATURE_STORAGE;
		if ( empty( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = 0;
		}
		$LITERATURE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'literature_storage_concat' ) ) {
	/**
	 ...
	 */
	function literature_storage_concat( $var_name, $value ) {
		global $LITERATURE_STORAGE;
		if ( empty( $LITERATURE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'literature_storage_get_array' ) ) {
	/**
	 ...
	 */
	function literature_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $LITERATURE_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ? $LITERATURE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'literature_storage_set_array' ) ) {
	/**
	 ...
	 */
	function literature_storage_set_array( $var_name, $key, $value ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $LITERATURE_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$LITERATURE_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'literature_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function literature_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ) {
			$LITERATURE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$LITERATURE_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$LITERATURE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'literature_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function literature_storage_merge_array( $var_name, $key, $value ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$LITERATURE_STORAGE[ $var_name ] = array_merge( $LITERATURE_STORAGE[ $var_name ], $value );
		} else {
			$LITERATURE_STORAGE[ $var_name ][ $key ] = array_merge( $LITERATURE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'literature_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function literature_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			literature_array_insert_after( $LITERATURE_STORAGE[ $var_name ], $after, $key );
		} else {
			literature_array_insert_after( $LITERATURE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'literature_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function literature_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			literature_array_insert_before( $LITERATURE_STORAGE[ $var_name ], $before, $key );
		} else {
			literature_array_insert_before( $LITERATURE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'literature_storage_push_array' ) ) {
	/**
	 ...
	 */
	function literature_storage_push_array( $var_name, $key, $value ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $LITERATURE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ) {
				$LITERATURE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $LITERATURE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'literature_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function literature_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $LITERATURE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $LITERATURE_STORAGE[ $var_name ] ) && is_array( $LITERATURE_STORAGE[ $var_name ] ) && count( $LITERATURE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $LITERATURE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $LITERATURE_STORAGE[ $var_name ][ $key ] ) && is_array( $LITERATURE_STORAGE[ $var_name ][ $key ] ) && count( $LITERATURE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $LITERATURE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'literature_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function literature_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ) {
			$LITERATURE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$LITERATURE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'literature_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function literature_storage_concat_array( $var_name, $key, $value ) {
		global $LITERATURE_STORAGE;
		if ( ! isset( $LITERATURE_STORAGE[ $var_name ] ) ) {
			$LITERATURE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $LITERATURE_STORAGE[ $var_name ][ $key ] ) ) {
			$LITERATURE_STORAGE[ $var_name ][ $key ] = '';
		}
		$LITERATURE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'literature_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function literature_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $LITERATURE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $LITERATURE_STORAGE[ $var_name ] ) ? $LITERATURE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $LITERATURE_STORAGE[ $var_name ] ) ? $LITERATURE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'literature_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function literature_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $LITERATURE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $LITERATURE_STORAGE[ $var_name ]->$prop ) ? $LITERATURE_STORAGE[ $var_name ]->$prop : $default;
	}
}
