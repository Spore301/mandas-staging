 ...
 *
 * @package LITERATURE
 * @since LITERATURE 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'LITERATURE_THEME_FREE' ) ) {
	define( 'LITERATURE_THEME_FREE', false );
}
if ( ! defined( 'LITERATURE_THEME_FREE_WP' ) ) {
	define( 'LITERATURE_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'LITERATURE_THEME_ALLOW_UPDATE' ) ) {
	define( 'LITERATURE_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'LITERATURE_ALLOW_SKINS' ) ) {
	define( 'LITERATURE_ALLOW_SKINS', true );
}
if ( ! defined( 'LITERATURE_DEFAULT_SKIN' ) ) {
	define( 'LITERATURE_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'LITERATURE_REMEMBER_SKIN' ) ) {
	define( 'LITERATURE_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['LITERATURE_STORAGE'] = array(
...
if ( ! function_exists( 'literature_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function literature_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = literature_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'literature_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'literature_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function literature_trx_updater_get_theme_info( $info ) {
		$engine = literature_storage_get( 'engine' );
...
if ( ! function_exists( 'literature_original_theme_slug' ) ) {
	add_filter( 'literature_filter_original_theme_slug', 'literature_original_theme_slug' );
	/**
	 ...
	 *
	 * @hooked 'literature_filter_original_theme_slug'
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function literature_original_theme_slug( $theme_slug = '' ) {
		return 'literature';
	}
}
...
if ( ! function_exists( 'literature_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'literature_importer_set_options', 9 );
	/**
	 ...
	 */
	function literature_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( literature_get_protocol() . ':' . literature_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( literature_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'literature_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'literature_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function literature_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( literature_get_protocol() . ':' . literature_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'literature_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'literature_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function literature_customizer_theme_setup1() {
		literature_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'literature_filter_allow_fullscreen' return false)
