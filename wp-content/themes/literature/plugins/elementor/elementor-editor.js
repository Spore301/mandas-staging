...
jQuery( document ).ready(
	function() {
		...
		literature_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="literature_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.literature_post_editor' );
						if ( link.length > 0 ) {
							literature_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.literature_post_editor', function(e) {
						literature_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('literature_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function literature_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
