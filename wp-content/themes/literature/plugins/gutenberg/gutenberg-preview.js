...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			literature_create_observer( 'literature-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				literature_gutenberg_editor_init();
			} );
		} else {
		 	literature_gutenberg_editor_init();
		}
	...
	function literature_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'literature_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'literature_inited' ) ) {
	...
	// Init on page load
	function literature_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = literature_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + LITERATURE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + LITERATURE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in LITERATURE_STORAGE['override_classes'] ) {
				$obj.addClass( LITERATURE_STORAGE['override_classes'][i].replace( '%s', LITERATURE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( literature_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && LITERATURE_STORAGE['sidebar_position'] != 'hide' ) {
				if ( literature_apply_filters( 'literature_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'literature_inited' );
		}

		// Widgets Editor
		//----------------------------------------------------------
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			if ( literature_apply_filters( 'literature_filter_add_scheme_to_widgets_editor', true ) ) {
				// Create an observer to add the class 'scheme_xxx' to the each new sidebar
				literature_remove_observer( 'literature-check-editor-styles-wrapper' );
				literature_create_observer( 'literature-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'literature_inited' );
	}
} );
