...
// Return a list of body classes for the layouts in the preview mode and in the editor mode
...
// Clear the Elementor cache after the layouts created
if ( ! function_exists( 'trx_addons_cpt_layouts_elm_layouts_create' ) ) {
	add_action( 'trx_addons_action_create_layouts', 'trx_addons_cpt_layouts_elm_layouts_create', 10, 1 );
	function trx_addons_cpt_layouts_elm_layouts_create( $layouts ) {
		if ( trx_addons_exists_elementor() && class_exists( '\Elementor\Plugin' ) && is_object( \Elementor\Plugin::instance()->files_manager ) ) {
...
// Collect and inline V4 atomic widgets' local styles for a given post_id.
// Elementor's get_builder_content() inlines only classic Post_CSS (_elementor_css),
// while atomic styles (Editor V4) are produced by a separate pipeline that runs once,
// on 'wp_enqueue_scripts', only for the main post. For nested CPT layouts this pipeline
// never fires — so we register the layout id in Atomic_Styles_Manager here, force it to
// render and enqueue, then swap the newly-enqueued <link>s for an inline <style> right
// before the layout's HTML (no FOUC, no dependency on late wp_footer).
//------------------------------------------------------------------------
if ( ! function_exists( 'trx_addons_cpt_layouts_elm_get_atomic_inline_styles' ) ) {
	function trx_addons_cpt_layouts_elm_get_atomic_inline_styles( $post_id, $return_inline_css = false ) {
		static $inlined = array();
		$post_id = (int) $post_id;
		if ( $post_id <= 0 || isset( $inlined[ $post_id ] ) ) {
			return '';
		}
		if ( ! class_exists( '\Elementor\Modules\AtomicWidgets\Styles\Atomic_Styles_Manager' ) ) {
			return '';
		}
		if ( $return_inline_css ) {
			global $wp_styles;
			if ( ! ( $wp_styles instanceof \WP_Styles ) ) {
				return '';
			}
			// Snapshot currently registered style handles so we can detect the ones added by the atomic pipeline below.
			$before = is_array( $wp_styles->registered )
					? array_fill_keys( array_keys( $wp_styles->registered ), true )
					: array();
		}

		$inlined[ $post_id ] = true;

		// Register the layout post id with Atomic_Styles_Manager and force it to render + enqueue.
		// Repeat calls are cheap: Cache_Validity inside the manager skips already-rendered post ids.
		do_action( 'elementor/post/render', $post_id );
		do_action( 'elementor/frontend/after_enqueue_post_styles' );

		if ( ! $return_inline_css && ! did_action( 'wp_print_footer_scripts' ) ) {
			return '';
		}

		// Pick up the newly registered handles that belong to this post id, inline their CSS and drop the enqueue.
		$css = '';
		$prefix = 'local-' . $post_id . '-';
		$upload_dir = wp_upload_dir();
		$base_path = trailingslashit( $upload_dir['basedir'] ) . 'elementor/css/';

		foreach ( array_keys( $wp_styles->registered ) as $handle ) {
			if ( isset( $before[ $handle ] ) || strpos( $handle, $prefix ) !== 0 ) {
				continue;
			}
			$file_path = $base_path . $handle . '.css';
			if ( is_file( $file_path ) ) {
				$file_css = @file_get_contents( $file_path );
				if ( ! empty( $file_css ) ) {
					$media = ! empty( $wp_styles->registered[ $handle ]->args )
								? (string) $wp_styles->registered[ $handle ]->args
								: 'all';
					$css .= ( $media === '' || $media === 'all' )
								? $file_css
								: '@media ' . $media . '{' . $file_css . '}';
				}
			}
			// Don't emit the <link> — the CSS is inlined alongside the layout HTML.
			wp_dequeue_style( $handle );
			wp_deregister_style( $handle );
		}

		return $css !== ''
				? '<style id="trx-addons-elementor-atomic-' . $post_id . '">' . $css . '</style>'
				: '';
	}
}
			...
			if ( $cur_page_built_with_elementor ) {
				...
				if ( trx_addons_sc_stack_check('show_layout') ) {
					...
					if ( $allow_recursive_layouts || ! trx_addons_sc_stack_check('show_layout_elementor') ) {
						...
						// The rendered template's stylesheet is enqueued with 'elementor-frontend'
						// as a dependency. When this method is called before Elementor's own
						// 'wp_enqueue_scripts' hook fires (e.g. during an Elementor editor
						// reload), that handle is not yet registered and WordPress emits a
						// _doing_it_wrong() notice. Register a stub up-front; Elementor's real
						// registration will overwrite it when its hook runs.
						...
						// Generate content of the layout
						$post_content = \Elementor\Plugin::instance()->frontend->get_builder_content( $post_id, $inline_css ); // || $is_edit_mode

						// Inline V4 atomic widgets' local styles for this layout — get_builder_content() handles only classic Post_CSS
						if ( ! empty( $post_content ) && strpos( $post_content, 'e-atomic-element' ) !== false ) {
							$atomic_css = trx_addons_cpt_layouts_elm_get_atomic_inline_styles( $post_id );
							if ( $atomic_css !== '' ) {
								$post_content = $atomic_css . $post_content;
							}
						}
