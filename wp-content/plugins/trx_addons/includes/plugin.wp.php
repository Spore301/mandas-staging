...
if ( ! function_exists( 'trx_addons_is_post_edit' ) ) {
	...
	function trx_addons_is_post_edit( $action = 'edit' ) {
		...
		return ( trx_addons_check_url( 'post.php' )
					&& ! empty( $_GET['action'] )
					&& $_GET['action'] == $action
					&& ! in_array( $post_type, apply_filters( 'trx_addons_filter_is_post_edit_exclude', array( 'shop_coupon' ) ) )
				)
				||
				( $action == 'edit'
					&& (
						( trx_addons_check_url( 'post-new.php' ) && use_block_editor_for_post_type( $post_type ) )
						||
